(async function load(){
  ...
  async function cacheExist(category){  
    if(category) {
      ...
      window.localStorage.setItem(`${category}List`, JSON.stringify(data));
      return data;
    } else {
  ...
  const x = renderPage();
  // time to clear the localStorage and fetch new data
  const interval = 10000;

  const clearStorage = setInterval(() => {
  ...
  function showModal($element) {
    ...
    try{
      ...
      const data = findUserByUuid(usersList, uuid)
      debugger
